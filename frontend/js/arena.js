...
const characters = {
    'Sōgō no Shisha': { attack: 80, defense: 60, special: 'Präzisionsschlag' },
    'Ayatsurishi': { attack: 70, defense: 70, special: 'Schattendoppelgänger' },
    'Bedy Yamiko': { attack: 150, defense: 100, special: 'Drachenklaue' },
    'Utsushi no Oni': { attack: 200, defense: 200, special: 'Dämonische Reflexion' },
    'Kōgei no Shinobi': { attack: 1000, defense: 5000, special: 'Tödliche Präzision' }
};
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    selectedCharacter = localStorage.getItem('selectedCharacter') || 'Sōgō no Shisha';
    
    // Wähle einen zufälligen Gegner
    const availableEnemies = Object.keys(characters).filter(char => char !== selectedCharacter);
...
function setupArena() {
    ...
    if (playerFighter) {
        ...
        if (playerImage) {
            playerImage.src = `images/zustandgesund/${selectedCharacter.replace(/\s+/g, '_')}.jpg`;
    ...
    if (enemyFighter) {
        ...
        if (enemyImage) {
            enemyImage.src = `images/zustandgesund/${enemyCharacter.replace(/\s+/g, '_')}.jpg`;
...
function updateHealthBars() {
    ...
    // Aktualisiere den Zustand der Kämpfer basierend auf Gesundheit
    updateFighterState('playerFighter', playerHealth);
    updateFighterState('enemyFighter', enemyHealth);
}

/**
 * Aktualisiert den visuellen Zustand eines Kämpfers basierend auf seiner Gesundheit
 * @param {string} fighterId - ID des Kämpfer-Elements
 * @param {number} health - Aktuelle Gesundheit des Kämpfers
 */
function updateFighterState(fighterId, health) {
    const fighter = document.getElementById(fighterId);
    if (!fighter) return;
    
    // Entferne alle Zustandsklassen
    fighter.classList.remove('health-normal', 'health-injured', 'health-critical');
    
    // Setze die entsprechende Zustandsklasse
    if (health > 40) {
    ...
    // Aktualisiere das Bild basierend auf dem Gesundheitszustand
    const fighterImage = fighter.querySelector('img');
    if (fighterImage) {
        const characterName = fighterId === 'playerFighter' ? selectedCharacter : enemyCharacter;
        let imageState = '';
        ...
        const newImageSrc = `images/${imageState}/${characterName.replace(/\s+/g, '_')}.jpg`;
...
function startBattle() {
    ...
    addLogMessage(`Der Kampf zwischen ${selectedCharacter} und ${enemyCharacter} beginnt!`, 'system');
...
function performAttack(attackType) {
    ...
    setTimeout(function() {
        ...
        // Kampflog-Nachricht
        addLogMessage(`Du führst ${getAttackName(attackType)} aus und verursachst ${damage} Schaden!`, 'player');
...
function enemyAttack() {
    ...
    setTimeout(function() {
        ...
        // Kampflog-Nachricht
        addLogMessage(`${enemyCharacter} führt ${getAttackName(attackType)} aus und verursacht ${damage} Schaden!`, 'enemy');
